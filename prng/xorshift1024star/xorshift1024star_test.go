...
var datadir = filepath.Join("..", "..", "data", "xorshift1024star")
var longTest bool
...
func Test_Xorshift1024star_GetSetSeed(t *testing.T) {
	assert := assert.New(t)

	seeds := []uint64{1, 5, 10, 1024, 200000}
	r := xorshift1024star.New(0)
...
func Test_Xorshift1024star_GetSetState(t *testing.T) {
	assert := assert.New(t)
	...
	r1 := xorshift1024star.New(0)
	for i := 0; i < 10; i++ {
		_ = r1.Uint64()
	}
	r2 := xorshift1024star.New(0)
	...
	_ = binary.Write(buf, binary.LittleEndian, []byte("xorshift1024start"))
	assert.Panics(func() {
		r1 := xorshift1024star.New(0)
		r1.SetState(buf.Bytes())
	})
	buf = new(bytes.Buffer)
	_ = binary.Write(buf, binary.LittleEndian, []byte("xorshift1024star"))
...
func Test_Xorshift1024star_Uint64(t *testing.T) {
	e := xorshift1024star.New(0)
	filenames := prngtest.GetDataFiles(datadir, "*uint64*.txt")
	prngtest.CompareDraws(t, e, filenames, longTest)
}

func Test_Xorshift1024star_Float64(t *testing.T) {
	e := xorshift1024star.New(0)
	filenames := prngtest.GetDataFiles(datadir, "*float64*.txt")
	prngtest.CompareDraws(t, e, filenames, longTest)
}

func Test_Xorshift1024star_Float64OO(t *testing.T) {
	e := xorshift1024star.New(0)
...
// Benchmarks
func Benchmark_Xorshift1024star_Uint64(b *testing.B) {
...
func Benchmark_Xorshift1024star_Float64(b *testing.B) {
	rng := xorshift1024star.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Float64()
	}
...
func Benchmark_Xorshift1024star_Float64OO(b *testing.B) {
	rng := xorshift1024star.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Float64()
	}
