...
var datadir = filepath.Join("..", "..", "data", "xorshift128plus")
var longTest bool
...
func Test_Xorshift128Plus_GetSetSeed(t *testing.T) {
	assert := assert.New(t)
...
func Test_Xorshift128Plus_GetSetState(t *testing.T) {
	assert := assert.New(t)
	...
	r1 := xorshift128plus.New(0)
	for i := 0; i < 10; i++ {
		_ = r1.Uint64()
	}
	r2 := xorshift128plus.New(0)
	...
	_ = binary.Write(buf, binary.LittleEndian, []byte("xorshift128plust"))
	assert.Panics(func() {
		r1 := xorshift128plus.New(0)
		r1.SetState(buf.Bytes())
	})
	buf = new(bytes.Buffer)
	_ = binary.Write(buf, binary.LittleEndian, []byte("xorshift128plus"))
...
func Test_Xorshift128Plus_Uint64(t *testing.T) {
	e := xorshift128plus.New(0)
	filenames := prngtest.GetDataFiles(datadir, "*uint64*.txt")
	prngtest.CompareDraws(t, e, filenames, longTest)
}

func Test_Xorshift128Plus_Float64(t *testing.T) {
	e := xorshift128plus.New(0)
	filenames := prngtest.GetDataFiles(datadir, "*float64*.txt")
	prngtest.CompareDraws(t, e, filenames, longTest)
}

func Test_Xorshift128Plus_Float64OO(t *testing.T) {
	e := xorshift128plus.New(0)
...
// Benchmarks
func Benchmark_Xorshift128Plus_Uint64(b *testing.B) {
	rng := xorshift128plus.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Uint64()
	}
}

func Benchmark_Xorshift128Plus_Float64(b *testing.B) {
	rng := xorshift128plus.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Float64()
	}
...
func Benchmark_Xorshift128Plus_Float64OO(b *testing.B) {
	rng := xorshift128plus.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Float64()
	}
