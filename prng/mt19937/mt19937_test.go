...
var datadir = filepath.Join("..", "..", "data", "mt19937")
var longTest bool
...
func Test_MT19937_GetSetSeed(t *testing.T) {
	assert := assert.New(t)
...
func Test_MT19937_GetSetState(t *testing.T) {
	assert := assert.New(t)
	...
	// Checking that the streams remain same after
	...
	_ = binary.Write(buf, binary.LittleEndian, []byte("mt19937"))
...
func Benchmark_MT19937_Float64OO(b *testing.B) {
	rng := mt19937.New(0)
	for i := 0; i < b.N; i++ {
		_ = rng.Float64OO()
	}
